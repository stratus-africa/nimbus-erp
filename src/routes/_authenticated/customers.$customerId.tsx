...
export const Route = createFileRoute("/_authenticated/customers/$customerId")({
  head: () => ({ meta: [{ title: "Customer — Nimbus ERP" }] }),
...
function CustomerDetailsPage() {
  ...
  const { data: invoices } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("invoices")
        .select("id, invoice_number, invoice_date, due_date, status, total, balance_due, created_at")
  ...
  const totalIncome = (invoices ?? [])
    .filter((i: any) => i.status !== "cancelled" && i.status !== "draft")
    .reduce((s: number, i: any) => s + Number(i.total ?? 0), 0);

  const filterLabel = FILTERS.find((f) => f.key === filter)?.label ?? "Active Customers";
  const paymentTermsDays = customer?.payment_terms_days ?? 0;
  const paymentTermsLabel = paymentTermsDays === 0 ? "Due on Receipt" : `Net ${paymentTermsDays}`;

  // audit timeline from invoices/payments
  const timeline = useMemo(() => {
    const events: { ts: string; title: string; detail?: string }[] = [];
    for (const inv of invoices ?? []) {
      events.push({
        ts: inv.created_at ?? inv.invoice_date,
        title: "Invoice created",
        detail: `Invoice ${inv.invoice_number} • ${formatCurrency(inv.total, currency)}`,
      });
    }
    for (const p of payments ?? []) {
      events.push({
        ...
        detail: `${p.invoices?.invoice_number ?? "—"} • ${formatCurrency(p.amount, currency)}`,
      });
    }
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className="w-[320px] shrink-0 border-r bg-card flex flex-col">
          ...
          <div className="overflow-y-auto">
            ...
            {!filtered.length && (
              <div className="px-4 py-10 text-center text-sm text-muted-foreground">No customers.</div>
            )}
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0 overflow-y-auto bg-background">
          {!customer ? (
            <div className="grid h-full place-items-center text-sm text-muted-foreground">
              {customers ? "Customer not found." : "Loading…"}
            </div>
          ) : (
            <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      <div className="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Sales</div>
                      <DropdownMenuItem onClick={() => navigate({ to: "/invoices/new" })}>Invoice</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => navigate({ to: "/payments-received/new" })}>Customer Payment</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => navigate({ to: "/quotes/new" })}>Quote</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => navigate({ to: "/sales-orders/new" })}>Sales Order</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <Button variant="outline" size="sm" className="h-8 gap-1">
                    More <ChevronDown className="h-3.5 w-3.5" />
                  </Button>
                  <Link to="/customers" className="ml-1 text-muted-foreground hover:text-foreground">
                    <X className="h-5 w-5" />
                  </Link>
                </div>
              </div>
              ...
              <Tabs defaultValue="overview" className="px-6 pt-3">
                ...
                <TabsContent value="overview" className="mt-0 pt-6">
                  <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <div className="space-y-5">
                      <div>
                        <div className="text-sm font-medium">{customer.company_name ?? customer.name}</div>
                      </div>
                      ...
                      <Section title="ADDRESS">
                        <Field label="Billing Address" value={customer.billing_address ?? "No Billing Address"} actionLabel="New Address" />
                        <Field label="Shipping Address" value={customer.shipping_address ?? "No Shipping Address"} actionLabel="New Address" />
                      </Section>
                      ...
                      <Section title={`CONTACT PERSONS (${customer.contact_person ? 1 : 0})`}>
                        {customer.contact_person ? (
                    ...
                    <div className="space-y-5">
                      ...
                      <div>
                        <div className="mb-2 text-sm font-medium">Receivables</div>
                        <div className="overflow-hidden rounded-md border">
                          <table className="w-full text-sm">
                            <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
                              <tr>
                                <th className="px-3 py-2 text-left font-medium">Currency</th>
                                <th className="px-3 py-2 text-right font-medium">Outstanding Receivables</th>
                      ...
                      <div>
                        ...
                        <div className="rounded-md border p-4">
                          ...
                          <div className="mt-1 text-2xl font-semibold tabular-nums">{formatCurrency(totalIncome, currency)}</div>
                        </div>
                      </div>
                ...
                <TabsContent value="transactions" className="pt-6 space-y-6">
                  ...
                  <div>
                    <div className="mb-2 text-sm font-medium">Payments Received</div>
                    {payments?.length ? (
                      <div className="overflow-hidden rounded-md border">
                        <Table>
                          <TableHeader>
