...
const DEFAULT_NOTES =
  "Order Rates are VAT Exclusive Unless Stated\nThank you for your business.";
const DEFAULT_TERMS =
  "1. Payment terms apply as agreed.\n2. Goods remain the property of the seller until paid in full.";
...
export function SalesOrderFormPage({
  ...
  const [soNumber, setSoNumber] = useState(initial?.so_number ?? "");
  const [reference, setReference] = useState(initial?.reference ?? "");
  const [soDate, setSoDate] = useState(initial?.so_date ?? todayIso());
  const [expectedDate, setExpectedDate] = useState(initial?.expected_shipment_date ?? "");
  ...
  const { data: items } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("items")
        .select("id, name, selling_price, item_type")
        .eq("tenant_id", tenantId!)
        .is("deleted_at", null)
        .order("name");
      const rows = data ?? [];
  ...
  const { data: previewNumber } = useQuery({
    ...
    queryKey: ["preview-so-number", tenantId],
    queryFn: async () => {
      const { data } = await (supabase as any)
        ...
        .limit(1);
      const last = data?.[0]?.so_number ?? "SO-0000";
      const match = /^(\D*)(\d+)$/.exec(last);
      if (!match) return "SO-0001";
  ...
  useEffect(() => {
    if (previewNumber && !soNumber) setSoNumber(previewNumber);
  }, [previewNumber, soNumber]);
  ...
  const addLine = () =>
    setLines((ls) => [...ls, { item_id: null, description: "", quantity: 1, rate: 0, tax_rate: 0 }]);
  ...
  const save = useMutation({
    mutationFn: async (sendAfter: boolean) => {
      ...
      const basePayload: any = {
        ...
        so_date: soDate,
        expected_shipment_date: expectedDate || null,
        status: sendAfter ? "confirmed" : "draft",
        subtotal,
        tax_total: taxTotal,
        total,
      ...
      if (isEdit && editId) {
        const { error: ue } = await (supabase as any)
          .from("sales_orders")
          .update({ ...basePayload, so_number: soNumber })
          .eq("id", editId);
        if (ue) throw ue;
        await (supabase as any).from("sales_order_lines").delete().eq("sales_order_id", editId);
        soId = editId;
      } else {
        ...
        const { data: inserted, error } = await (supabase as any)
          .from("sales_orders")
          .insert({ ...basePayload, so_number: (num as any) ?? soNumber })
          .select("id")
          .single();
  ...
  return (
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="flex items-center justify-between border-b bg-card px-6 py-3.5">
        <div className="flex items-center gap-2">
          <div className="grid h-6 w-6 place-items-center rounded border text-muted-foreground">
            <span className="text-[10px]">SO</span>
          </div>
          <h1 className="text-lg font-semibold">{isEdit ? `Edit Sales Order ${soNumber}` : "New Sales Order"}</h1>
        </div>
        <Link to="/sales-orders" className="text-rose-500 hover:text-rose-600" aria-label="Close">
          <X className="h-5 w-5" />
        </Link>
      </div>
        ...
        <div className="w-full space-y-0 px-6 py-6">
            ...
            <Field label="Location">
              <div>
                <Select value={location} onValueChange={setLocation}>
                  ...
                  <SelectContent><SelectItem value="Head Office">Head Office</SelectItem></SelectContent>
                </Select>
          ...
          <section className="space-y-4 border-b py-6">
            <Field label="Sales Order#" required>
              <div className="relative max-w-xs">
                <Input value={soNumber} onChange={(e) => setSoNumber(e.target.value)} className="h-9 pr-9" />
                <button type="button" className="absolute right-2 top-1/2 -translate-y-1/2 text-sky-500 hover:text-sky-600">
                  <Settings2 className="h-4 w-4" />
                </button>
              </div>
            </Field>
            <Field label="Reference#">
              <Input value={reference} onChange={(e) => setReference(e.target.value)} className="h-9 max-w-xs" />
            </Field>
            <Field label="Sales Order Date" required>
              <div className="flex flex-wrap items-center gap-6">
                <div className="relative">
                  <Input type="date" value={soDate} onChange={(e) => setSoDate(e.target.value)} className="h-9 w-56 pr-9" />
                  <CalendarIcon className="pointer-events-none absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                </div>
                <div className="flex items-center gap-3">
                  <label className="text-sm">Expected Shipment Date</label>
                  <div className="relative">
                    <Input type="date" value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} className="h-9 w-56 pr-9" />
                    <CalendarIcon className="pointer-events-none absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  </div>
                </div>
              </div>
          ...
          <section className="border-b py-6">
            <Field label="Salesperson">
              <Select value={salesperson} onValueChange={setSalesperson}>
                <SelectTrigger className="h-9 max-w-xs"><SelectValue placeholder="Select or Add Salesperson" /></SelectTrigger>
          ...
          <section className="border-b py-6">
            <Field label="Description" hint>
              <Textarea
                ...
                placeholder="Let your customer know what this Sales Order is for"
                ...
                <thead>
                  <tr className="border-y bg-muted/20 text-[11px] uppercase tracking-wide text-muted-foreground">
                    ...
                    <th className="w-32 px-3 py-2 text-right font-medium">
                      <span className="inline-flex items-center gap-1">Rate <Settings2 className="h-3 w-3" /></span>
                    </th>
                      ...
                      <tr key={idx} className="border-b last:border-b-0">
                        <td className="px-3 py-3 align-top">
                          <div className="flex items-start gap-2">
                            <button className="mt-1.5 text-muted-foreground hover:text-foreground"><GripVertical className="h-4 w-4" /></button>
                            ...
                            <div className="flex-1">
                              <ItemPicker
                                ...
                                onDescriptionChange={(v) => updateLine(idx, { item_id: null, description: v })}
                              />
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-3 py-3 align-top">
                          <Select value={String(l.tax_rate)} onValueChange={(v) => updateLine(idx, { tax_rate: parseFloat(v) })}>
                            <SelectTrigger className="h-9"><SelectValue placeholder="Select a VAT" /></SelectTrigger>
                            <SelectContent>
                        ...
                        <td className="px-3 py-3 align-top text-right tabular-nums">{amount.toFixed(2)}</td>
                        <td className="px-2 py-3 align-top text-right">
                          {lines.length > 1 && (
                            <button onClick={() => removeLine(idx)} className="text-muted-foreground hover:text-rose-500">
            ...
            <div className="mt-4 flex flex-wrap items-start justify-between gap-6">
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={addLine}
                  ...
                  <Plus className="h-4 w-4" /> Add New Row <ChevronDown className="ml-1 h-3.5 w-3.5" />
                </Button>
                <Button variant="outline" size="sm" className="h-9 gap-1.5 border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100">
          ...
          <section className="border-t pt-6">
            <Field label="Customer Notes">
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="min-h-[64px] max-w-md" />
            </Field>
          </section>
          ...
          <section className="mt-6 grid gap-6 rounded-md bg-muted/30 p-5 md:grid-cols-2">
            <div>
              ...
              <Textarea value={terms} onChange={(e) => setTerms(e.target.value)} className="min-h-[120px] bg-background" />
            </div>
            <div>
              <h3 className="mb-2 text-sm font-medium">Attach File(s) to Sales Order</h3>
              <Button variant="outline" size="sm" className="h-9 gap-2">
                <Upload className="h-4 w-4" /> Upload File <ChevronDown className="h-3.5 w-3.5" />
              </Button>
              <p className="mt-2 text-xs text-muted-foreground">You can upload a maximum of 5 files, 10MB each</p>
            </div>
          </section>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-t bg-card px-6 py-3">
        <Button variant="outline" size="sm" className="h-9" disabled={save.isPending} onClick={() => save.mutate(false)}>
          Save as Draft
        </Button>
        <Button size="sm" className="h-9 bg-emerald-600 text-white hover:bg-emerald-700"
          ...
          {save.isPending ? "Saving…" : "Save and Confirm"}
        </Button>
        <Button variant="outline" size="sm" className="h-9" onClick={() => navigate({ to: "/sales-orders" })}>
          Cancel
        </Button>
      </div>
    </div>
...
function Field({ label, required, hint, children }: { label: string; required?: boolean; hint?: boolean; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[160px_1fr] items-start gap-4">
      <label className={`pt-2 text-sm ${required ? "text-rose-500" : "text-foreground"} flex items-center gap-1`}>
        {label}
        {required && <span>*</span>}
        {hint && <span className="grid h-3.5 w-3.5 place-items-center rounded-full bg-muted text-[9px] text-muted-foreground">i</span>}
      </label>
      <div>{children}</div>
    </div>
  );
...
function ItemPicker({
  value, description, items, onPick, onDescriptionChange,
}: {
  value: string | null; description: string;
  items: Array<{ id: string; name: string; selling_price: number | null }>;
  onPick: (id: string) => void; onDescriptionChange: (v: string) => void;
}) {
    ...
    <div className="relative">
      <input
        ...
        onChange={(e) => {
          ...
          else { setQuery(e.target.value); onDescriptionChange(e.target.value); }
        }}
