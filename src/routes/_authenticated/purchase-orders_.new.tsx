...
export const Route = createFileRoute("/_authenticated/purchase-orders_/new")({
  head: () => ({ meta: [{ title: "New Purchase Order — Nimbus ERP" }] }),
...
const DEFAULT_NOTES =
  "Rates are VAT Exclusive Unless Stated.\nKindly confirm receipt of this purchase order.";
const DEFAULT_TERMS =
  "1. Delivery: Goods are to be delivered within the agreed timeline.\n2. Payment: Payment terms as agreed in the supplier contract.\n3. Inspection: All goods are subject to inspection upon receipt.";
...
function NewPurchaseOrderPage() {
  ...
  const { data: suppliers } = useQuery({
    enabled: !!tenantId,
    queryKey: ["suppliers-pick", tenantId],
    queryFn: async () => {
  ...
  const { data: items } = useQuery({
    enabled: !!tenantId,
    queryKey: ["items-pick-po", tenantId],
    queryFn: async () => {
      const { data } = await supabase
        .from("items")
  ...
  // Preview the next PO number (display-only; real number is fetched on save)
  const { data: previewNumber } = useQuery({
    enabled: !!tenantId,
    queryKey: ["preview-po-number", tenantId],
    queryFn: async () => {
      const { data } = await supabase
        ...
        .limit(1);
      const last = data?.[0]?.po_number ?? "PO-202400";
      const match = /^(\D*)(\d+)$/.exec(last);
      if (!match) return "PO-202401";
  ...
  useEffect(() => {
    if (previewNumber && !poNumber) setPoNumber(previewNumber);
  }, [previewNumber, poNumber]);
  ...
  const pickItem = (idx: number, itemId: string) => {
    ...
    updateLine(idx, {
      item_id: it.id,
      description: it.name,
      // POs default to cost price for purchases
      rate: Number(it.cost_price ?? it.selling_price ?? 0),
    });
  };
  ...
  return (
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="flex items-center justify-between border-b bg-card px-6 py-3.5">
        ...
        <Link to="/purchase-orders" className="text-rose-500 hover:text-rose-600" aria-label="Close">
          <X className="h-5 w-5" />
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        <div className="mx-auto max-w-5xl space-y-0 px-6 py-6">
              ...
              <div className="flex items-center gap-0">
                <Select value={supplierId} onValueChange={setSupplierId}>
                  <SelectTrigger className="h-9 rounded-r-none border-r-0 focus:ring-1 focus:ring-sky-400">
                    <SelectValue placeholder="Select or add a supplier" />
                  </SelectTrigger>
                  <SelectContent>
                    {suppliers?.map((s: any) => (
          ...
          <section className="border-b py-6">
            <Field label="Buyer">
              <Select value={buyer} onValueChange={setBuyer}>
                <SelectTrigger className="h-9 max-w-xs">
                  <SelectValue placeholder="Select or Add Buyer" />
                </SelectTrigger>
                <SelectContent>
          ...
          <section className="border-b py-6">
            <Field label="Description" hint>
              <Textarea
                ...
                placeholder="Let your supplier know what this Purchase Order is for"
          ...
          <section className="mt-6 grid gap-6 rounded-md bg-muted/30 p-5 md:grid-cols-2">
            ...
            <div>
              <h3 className="mb-2 text-sm font-medium">Attach File(s) to Purchase Order</h3>
          ...
          <p className="mt-6 text-xs text-muted-foreground">
            ...
            your purchase orders by going to <em>Settings</em> → <em>Purchases</em> →{" "}
...
function ItemPicker({
  ...
  value: string | null;
  description: string;
  items: Array<{ id: string; name: string; cost_price: number | null; selling_price: number | null }>;
