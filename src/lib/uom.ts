...
export const UOM_CLASSES: UomClass[] = [
  "Unit",
  "Weight",
  "Length",
  "Volume",
 ...
 */
export function buildUomEngine(
  master: UomMaster[],
  ...
  function getByClass(uomClass: UomClass): UomMaster[] {
    return master
      .filter((u) => u.uom_class === uomClass && u.is_active)
  ...
  function checkCircular(
    ...
    while (queue.length > 0 && depth <= 6) {
      ...
      for (const node of queue) {
        ...
        if (node === fromUom) {
          return (
            `Circular conversion: adding "${fromUom} → ${toUom}" ` +
        ...
        for (const c of conversions) {
          if (
            c.from_uom === node &&
            (c.item_id === (itemId ?? null) || c.item_id === null)
          ) {
            next.push(c.to_uom);
          }
        }
 ...
 */
export async function dbConvert(
  qty: number,
  fromUom: string,
  toUom: string,
  itemId?: string | null
 ...
 */
export async function dbHasPath(
  fromUom: string,
  toUom: string,
  itemId?: string | null
): Promise<boolean> {
